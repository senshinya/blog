...
// term 留分类 id（机器可读），label 补上展示名给订阅器直接显示 ——
// 三语 feed 各自按其 locale 从对应词条表（MESSAGES[locale].category）取展示名
function getCategoryLabel(category: string | undefined, locale: keyof typeof MESSAGES) {
	if (!category)
		return undefined
	const categories = MESSAGES[locale].category
...
export async function buildAtomFeed(event: H3Event, locale: keyof typeof MESSAGES) {
	...
	const entries = posts.map(post => ({
		...
		category: { $term: post.categories?.[0], $label: getCategoryLabel(post.categories?.[0], locale) },
